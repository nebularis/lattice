...
| 3.7 | ADR-A18 Surface-to-MORK lowering engine (`tools/surface/lowering.py`) | Done, **unverified** — `lower_projection`, `lower_contract`, and `lower_all`/`link_dependencies` (the `mork:dependsOnMapping` graph) are implemented per ADR-A18/A19 and covered by new tests in `test_surface.py`, but this environment has no Python interpreter. Run `python -m unittest tools.surface.test_surface -v` after applying this patch elsewhere, before relying on it. |
...
| 4 / 15.4 | `fnd:GovernanceState` individuals undeclared | **Closed** — corrected 2026-09-18: the previous "Done" tag here was wrong. `docs/architecture/solution-architecture.md` already recorded this as a real, undeclared gap, and a repo-wide grep confirmed every reference to `fnd:Active` etc. was a forward reference, never a declaration. Now declared in `foundation/vocab/foundation-vocab.ttl` as part of the Phase 4 MORK governance work, which needed them to be real. |
| 3.8 | MORK governance and versioning (ADR-A22) | Done, **unverified** — Foundation import, `mork:GenerativeMapping` adopting `fnd:Governable`/`fnd:Version`, `mork:CompilationMode`, and three SHACL shapes in `mork/shapes/constraints.ttl` (previously empty). See `mork/docs/governance-and-versioning-migration.md` for the backward-compatibility story and verification plan; no SHACL engine or Python was available to run it. |
| 3.9 | MORK SPARQL/SHACL/SWRL compiler family for Eligibility (ADR-A23, ADR-A24) | Done, **unverified** — new `tools/mork_compilers/` package compiles `elg:IntervalCondition` through a shared IR into a `mork:QueryTemplate` (SPARQL), two SHACL shapes on one `mork:DataMapping`, and one `swrl:Imp` per required interval on another. New `mork/spec/Executable.ttl` (`exe:` vocabulary). No "native" backend — no definition of one exists in this repository. No profile-level aggregate artefact and no runtime result tracking either — both stated gaps, not silent omissions. Full scope record in `mork/docs/eligibility-executable-compiler.md`. |
...
### 3.7 Surface-to-MORK lowering engine — done, unverified
...
This has not been executed. The authoring environment has no Python interpreter (see repository memory notes), so `tools/surface/test_surface.py`'s new `ContractLoweringTests` and `LoweringDependencyTests` classes are reviewed by reading, not by a test run. Run `python -m unittest tools.surface.test_surface -v` once this patch is applied somewhere Python exists, and treat a first pass as part of review rather than as a regression check — the same caveat the rest of this test module already carries.
