...
The implementation has now been executed. `python3 -m unittest surface.test_surface -v` passes 54/54, including `ContractLoweringTests`, `LoweringDependencyTests`, Projection lowering, determinism, and MORK interop.

**No committed goldens.** `ontology/surface/execution/`'s convention is regenerated output, not hand-authored fixtures (see that directory's own README). No lowering output has been committed here for the same reason: a hand-computed "golden" `.ttl` file cannot be verified byte-accurate without running the compiler, and a wrong golden is worse than none. Regenerate with `python -m surface lower --contracts ontology/surface/examples/saas-subscription-arr-projection.ttl` once Python is available, and commit the result under `ontology/surface/execution/` at that point.
