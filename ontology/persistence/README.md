...
### 3a. `dal:onViolation` selects a reconciler, not a different guarded write

The guarded write (`key-claim-write`, guide §6.2) is the same for every `dal:onViolation` value: it always prevents the race at write time. What `dal:onViolation` actually selects is which **reconciler** operation the compiler generates for that constraint (guide §7.5, P7 — "the reconciler is never the only strategy, and it is never absent"), since a bulk load, an administrative `LOAD`, or a restore can all create a duplicate the guarded write never saw:

| `dal:onViolation` | Reconciler operation | What it does |
|---|---|---|
| `dal:Reject` (the baseline default when undeclared) | `key-claim-duplicate-audit` | Read-only: reports every over-claimed key |
| `dal:Merge` (needs `dal:mergeRelation`) | `key-claim-merge-rewrite` | Records `dal:mergeRelation` from every non-canonical owner to the lexicographically lowest owner IRI — a documented, deterministic, but arbitrary choice, never `owl:sameAs` (`dal:MergeRelationRequiredShape`) |
| `dal:Quarantine` | `key-claim-quarantine` | Copies every over-claimed key's owners into a dedicated quarantine graph for human review, deleting nothing |

None of the three resolves the duplicate automatically end to end: rewriting payload references to a merge's canonical IRI, and retiring a losing claim, stay with the caller, since only a claim's own owner may retire it (guide §6.2) and a reconciler has no such authority.

`dal:ClaimScheme` rotation (guide §6.1) is a second, independent axis: when a constraint's active schemes (`dal:Accepting`/`dal:Dual`) number two, the guarded write itself changes, to `key-claim-write-dual`, which guards and inserts both scheme versions' claim IRIs for the same owner in one operation, so no window opens where the key is claimed under only one version while the secret rotates.
...
The compiler resolves the two identity roles independently, `identity:EntityRole` and `identity:EventOccurrenceRole` (`persistence-compiler-iri-sync` Slice 3), and emits a self-contained minting recipe for each (`identity-minting` M1): every member above that a recipe needs is required, and missing ones are refused by name. `python -m persistence export-recipes` writes the recipes as JSON, for the minting libraries or any implementation of the [identity minting specification](../../docs/architecture/identity-minting-specification.md). The privacy profile is validated by SHACL and resolved by the compiler (`persistence-compiler-iri-sync` Slice 4): `dal:privacyClass`, `dal:erasureStrategy` and `dal:erasurePrecedence` each resolve as their own dimension, matching how the extension properties in §3 resolve, and `dal:epochAuthority` is promoted out of `dal:epochGuardScope`'s extras into its own dimension alongside them, for the same reason. The compiler refuses `dal:PersonalData` combined with `dal:NoErasure`, and refuses `dal:PersonalData` combined with a replay-capable receipt model (`dal:PatchLog`/`dal:SnapshotPerRevision`) unless `dal:perSubjectScoped true` or `dal:erasureStrategy dal:CryptoShred` is also declared, whichever profile node each comes from.

`ex:ClaimantEvents` above needs a `dal:digestScheme` because its namespace derivation is `dal:HashedTargetDerivation` — the namespace is itself an injective digest of the target's own IRI. A `dal:PositionDerivedEvent` profile whose namespace derivation is `dal:RegistryTokenDerivation` instead needs none: the namespace is a registry-allocated token, not digest-derived, so `dal:DigestSchemeRequiredShape` (and its compiler mirror) exempt exactly that combination (`persistence-compiler-iri-sync` Slice 5). See `examples/identity-minting-coverage.ttl`'s `ex:ShipmentEvents`.

Full fixture: [`examples/identity-epoch-privacy-profile.ttl`](examples/identity-epoch-privacy-profile.ttl), with two Slice 3 negative fixtures, [`examples/invalid-claimed-identity-without-key.ttl`](examples/invalid-claimed-identity-without-key.ttl) and [`examples/invalid-position-event-without-derivation.ttl`](examples/invalid-position-event-without-derivation.ttl). Slice 1 authored two epoch-only fixtures: [`examples/epoch-dataset-level-guard.ttl`](examples/epoch-dataset-level-guard.ttl) and [`examples/warning-epoch-unsafe-restore.ttl`](examples/warning-epoch-unsafe-restore.ttl). [`examples/append-stream-dataset-guard.ttl`](examples/append-stream-dataset-guard.ttl) covers an append-only stream under the dataset-level guard. [`examples/identity-minting-anchors.ttl`](examples/identity-minting-anchors.ttl) compiles to exactly the seven recipes of `contracts/identity/anchor-vectors.json`, one per recipe strategy, and [`examples/identity-minting-coverage.ttl`](examples/identity-minting-coverage.ttl) to four more that exercise the remaining pipelines, encodings, surrogate kinds and namespace derivations. Both feed the conformance vectors in `packages/minting/testdata`. Slice 2 added [`examples/extension-properties.ttl`](examples/extension-properties.ttl), every extension property declared on its own profile node, and [`examples/invalid-lagwindow-missing.ttl`](examples/invalid-lagwindow-missing.ttl), refused by `dal:LagWindowRequiredShape` and by the compiler. Slice 4 added [`examples/invalid-personaldata-no-erasure.ttl`](examples/invalid-personaldata-no-erasure.ttl), [`examples/invalid-personaldata-receipt-conflict.ttl`](examples/invalid-personaldata-receipt-conflict.ttl), and the compatible counterpart [`examples/privacy-receipt-compatible.ttl`](examples/privacy-receipt-compatible.ttl), whose `dal:PrivacyProfile` and `dal:ReceiptProfile` are declared on separate individuals sharing one `dal:appliesTo` scope, to show the cross-profile-class join catches that case too. Slice 5 added [`examples/uniqueness-merge-policy.ttl`](examples/uniqueness-merge-policy.ttl) and [`examples/uniqueness-quarantine-policy.ttl`](examples/uniqueness-quarantine-policy.ttl) (one `dal:onViolation` policy each), [`examples/invalid-merge-policy-no-relation.ttl`](examples/invalid-merge-policy-no-relation.ttl) (refused by `dal:MergeRelationRequiredShape` and by the compiler), and [`examples/claim-scheme-dual-rotation.ttl`](examples/claim-scheme-dual-rotation.ttl) (two `dal:ClaimScheme`s both `dal:Dual`, selecting the dual-guard write template).
