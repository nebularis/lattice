...
- **Extraction tooling should assert its section filter explicitly**, not infer it from position. This document's own extraction was tested by concatenating every ` ```turtle ` block found anywhere in the file, which silently pulled in §3's template illustration and would have pulled in §9's worked example too — both fixed in §3's instructions above, but a real `tools/` implementation of this extraction should hard-code the section allowlist (or, better, use a distinct fence tag like ` ```turtle-spec ` for genuine axiom content versus plain ` ```turtle ` for illustration, removing the ambiguity at the source rather than relying on a documented rule a script has to know to honour).
