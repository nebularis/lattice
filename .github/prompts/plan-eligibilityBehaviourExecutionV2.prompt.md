...
- [ontology/eligibility/README.md](ontology/eligibility/README.md), [ontology/behaviour/README.md](ontology/behaviour/README.md), [ontology/instrument/README.md](ontology/instrument/README.md) are all still genuinely empty (0 bytes) — clean-room authoring is straightforward, no adversarial procedure needed.
- **ontology/quantification/spec/quantification.ttl declares no `owl:imports`** despite its own README stating it imports Foundation and Vocabulary (§2) — the axioms haven't caught up with the design prose. **[ontology/party/spec/party.ttl](ontology/party/spec/party.ttl) imports only Foundation**, not Quantification, despite Quantification's README claiming Party imports it. This mismatch is a Gate 1 fix, not a new design decision.
...
- [docs/architecture/decisions](docs/architecture/decisions), [docs/validation-and-test-plan.md](docs/validation-and-test-plan.md), and [docs/operational-guidance.md](docs/operational-guidance.md) are all empty — no conflicting content to reconcile, just files to author.
...
1. ADR convention: docs/architecture/decisions/ADR-{id}-{slug}.md, MADR-lite shape (Status/Context/Decision/Consequences), indexed by docs/architecture/decisions/README.md.
...
3. Containment rule, stated once in docs/GOVERNANCE.md rather than repeated per-gate: public substrate directories stay domain-neutral and closed-brand-free; ontology/applied/deployment-specific content (e.g. a MERIDIAN dimension set) may be authored anywhere in the repo the user designates and is committed only at the user's discretion.
...
12. Author non-domain worked examples in ontology/eligibility/examples/ before mechanism prose.
13. Author ontology/eligibility/README.md: imports now include **Quantification directly**; match-strategy algebra with **`IntervalContainment` included** (using `qnt:Range`/`qnt:RangeSet`) and `IntervalOverlap` excluded (a law-coherence defect, independent of Quantification's existence); three-valued `Decision`; three named compatibility operations; wildcard rules; canonicalisation; `AdmissionProfile ⊑ Condition`; operational profiles E1–E6; laws L1–L8.
14. Extract ontology/eligibility/spec/eligibility.ttl, vocab, shapes via fence tags.
15. Populate ontology/eligibility/projection/party.ttl; consider ontology/eligibility/projection/quantification.ttl for the `IntervalContainment` binding.
16. Build E1/E2 conformance fixtures under ontology/eligibility/test/, including one exercising `IntervalContainment`.
...
19. Author ontology/instrument/README.md, spec, vocab, shapes.
20. Populate ontology/instrument/projection/party.ttl.
...
22. Author non-domain worked examples in ontology/behaviour/examples/ first.
23. Author ontology/behaviour/README.md: four tiers, TriggerDefinition split, authority/activation/selection policies, StateOccupancy with hypothetical-graph separation, effect payload contract, cascade/queue semantics.
...
25. Extract ontology/behaviour/spec, vocab, shapes.
26. Populate ontology/behaviour/projection/{eligibility,instrument,party,quantification}.ttl.
27. Populate ontology/behaviour/execution/{invalidation-policy,benchmark-pack,split-plan}.md.
