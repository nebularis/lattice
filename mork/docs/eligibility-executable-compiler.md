...
Date: 2026-09-18
Status: Accompanies [ADR-A23](../../docs/adr/ADR-A23-mork-compiler-family-completion-policy.md) and [ADR-A24](../../docs/adr/ADR-A24-eligibility-executable-semantics-backend-strategy.md)
Source design note: [surface/docs/MorkEnhancements.md](../../surface/docs/MorkEnhancements.md)

## What this is

`tools/mork_compilers/` and `mork/spec/Executable.ttl` implement delivery-plan Phase 5 (the MORK compiler family) narrowed to what MorkEnhancements.md's proposal and ADR-A24 actually call for first: a shared executable IR for Eligibility's `elg:IntervalCondition`, and three backend compilers reading it — SPARQL, SHACL, and SWRL.
...
2. Parse `mork/spec/Executable.ttl` and confirm it is consistent with `mork/spec/Mork.ttl` under a real OWL reasoner — the punned references to `elg:Condition`, `elg:Question`, `elg:Decision` etc. should resolve once Eligibility's own ontology is loaded alongside it, even though this file does not import Eligibility formally.
3. Execute the generated SPARQL query from `sparql_backend.compile_query_template` against `eligibility/examples/interval-containment.ttl` and confirm `ex:question-1` resolves to `"Permitted"`.
