...
- **Diagnostic codes are not implemented.** This ADR's consequence that "unsupported semantics... return undetermined with a diagnostic code" holds for the *outcome* (the SPARQL and SHACL backends both distinguish missing evidence from a failed containment check) but no `exe:Diagnostic`/diagnostic-code vocabulary exists yet — `mork/spec/Executable.ttl` deliberately omits it as unused vocabulary rather than declaring it ahead of any consumer. `Undetermined` is a bare decision value, not yet paired with a machine-readable reason code.

See `mork/docs/eligibility-executable-compiler.md` for the full scope record and verification plan. Not executed.
