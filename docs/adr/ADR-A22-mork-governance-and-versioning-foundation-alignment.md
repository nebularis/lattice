...
**Also found and fixed as a prerequisite:** `fnd:GovernanceState`'s four named individuals did not actually exist anywhere in the repository — `docs/architecture/solution-architecture.md` already recorded this as a known gap, and every reference to `fnd:Active` elsewhere (Surface's examples, Vocabulary's and Party's worked examples) was a forward reference. Declared now in `foundation/vocab/foundation-vocab.ttl`, since none of the above is meaningful without them.
