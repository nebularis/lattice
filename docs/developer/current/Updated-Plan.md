# LATTICE Platform — Agentic Development Plan (v0.1, for review)
...
contracts/                     # normative JSON Schema + OpenAPI; source of all generated types
  events/ surface/ mork/ pack/ plan/ projection/ query/ lineage/ openapi/
ontology/                      # existing layers, unchanged in role
  foundation/ vocabulary/ quantification/ party/ eligibility/ instrument/ behaviour/
  platform/                    # NEW: lattice: namespace (authority, commitSeq, lifecycle, provenance)
  governance/shapes/           # NEW content: authority + provenance governance shapes
mork/
platform/
  lattice-bom/
...
| Slice | Scope | Tests / validation |
|---|---|---|
| **P0.3.1** | Declare `fnd:GovernanceState` individuals (`Draft`/`Reviewed`/`Active`/`Superseded`) in `foundation/vocab/foundation-vocab.ttl` + `shapes/constraints.ttl` `sh:in`; closes G-34 part 1 | **L1**: SHACL rejects an unknown governance state; Vocabulary's worked example no longer forward-references |
...
| **P0.3.5** | `governance/shapes/authority.ttl`: any graph containing inferred/materialised triples without an authority annotation + `fnd:derivedFrom` + `(semanticHash, profileId)` is a governance failure (G-28, §4.13) | **L1**: positive/negative graph fixtures |
| **P0.3.6** | `governance/shapes/provenance.ttl`: every assertion graph has exactly one `ProvenanceScope` with one `cause` (provenance homogeneity, A65); `lattice:transactionTime` present (A67) | **L1**: multi-cause graph rejected |
...
| **P0.9.2** | Ontology/contract fixtures: a demo applied ontology derived from `examples/insure-o`, Surface Promotion/Index/Projection contracts, Eligibility profiles, Behaviour declarations incl. a capacity-tank aggregate | **L1**: all generated TTL parses and passes layer shapes |
...
| 5 | **Is the insurance applied ontology (`examples/insure-o`) an acceptable basis for the synthetic corpus**, or do we need a neutral second domain to prevent domain drift (per `GENAI_CONTRIBUTION.md`'s stated concern)? | If a second domain is required, P0.9.2 roughly doubles and every L5/L6 scenario runs in a matrix. |
