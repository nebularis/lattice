...
- **The claimed-key check** requires at least one uniqueness constraint on the target. It does not check that a constraint's key is the one the claim is minted from, because the vocabulary does not link an identity profile to a specific constraint. *Superseded on 2026-09-23 by `identity-minting` M1: `dal:claimsConstraint` now names the constraint exactly, and the interim check was replaced ([M1 VP](identity-minting-m1.md#contract-changes-non-weakening-rule)).*
