...
> - **Pattern K (uniqueness)** ↔ `dal:UniquenessConstraint` (`dal:keyProperty`, `dal:scopeProperty`, `dal:normalizePipeline`, `dal:onViolation`, `dal:minEnforcementLevel`). ADR-A51's IRI/identity policy (P0.1.3) already makes registry uniqueness structural for minting; cross-referenced below as the Core-tier case `dal:UniquenessConstraint`'s `dal:Strong` enforcement level assumes exists.
...
| **P0.1.3** | ADR-A51 IRI & identity policy (lineage IRI / revision IRI / alias; entity minting strategies) + `docs/architecture/iri-policy.md`. Cross-references `ontology/persistence`'s `dal:UniquenessConstraint` (Pattern K): structural registry uniqueness here is the Core-tier case its `dal:Strong` enforcement level assumes exists everywhere else | Does the revision-IRI form make registry uniqueness structural? Is `surrogate` minting adequately fenced? | P0.4 + P1.2 tests |
