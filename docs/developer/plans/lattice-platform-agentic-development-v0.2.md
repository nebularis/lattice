...
| **P0.1.3** | ADR-A51 IRI & identity policy (lineage IRI / revision IRI / alias; entity minting strategies) + `docs/architecture/iri-policy.md` | Does the revision-IRI form make registry uniqueness structural? Is `surrogate` minting adequately fenced? | P0.4 + P1.2 tests |
...
| **P0.1.5** | ADR-A65 provenance: named-graph-per-batch, provenance-homogeneous batches, `ProvenanceAssertion` opt-in | Is "one cause per graph" achievable for every write in the T1–T12 catalogue? | `CommitMetadata` required (P0.5) |
...
| **P0.2.7** | **ArchUnit + lint guardrail suite** (G1, G2, G5, G6, G7, G9, G10) and Python equivalents; `reuse lint` licence headers incl. new MPL-2.0 platform code | **L1**: each guardrail has a positive fixture (allowed) and a violating fixture (build fails). This is the slice that makes guardrails real — validate it hard |
...
| **P0.3.6** | `ontology/governance/shapes/provenance.ttl`: every assertion graph has exactly one `ProvenanceScope` with one `cause` (provenance homogeneity, A65); `lattice:transactionTime` present (A67) | **L1**: multi-cause graph rejected |
| **P0.3.7** | **Graph-name validator** library + grammar tests for the A54 layout convention (`urn:lattice:{tenant}:{env}:...`), including lineage/revision/alias forms from A51 | **L1/L2**: grammar accepts the full family table, rejects malformed, round-trips parse/format |
...
| **P0.5.2** | Core write surface: `WriteUnit`, `CommitMetadata` (mandatory), `apply`, `conditionalWrite(Precondition, WriteUnit)`, `putGraph`, `dropGraph(DropPolicy)`, `WriteResult(CommitSequence, transactionTime, guardSatisfied, stats)` | **L1**: a `WriteUnit` without `CommitMetadata` does not compile/constructs to failure; `guardSatisfied=false` leaves the dataset byte-identical |
...
| **P0.5.6** | **TCK: concurrency suite** (R-14) — N concurrent `conditionalWrite`s on one subject yield exactly one winner, N−1 clean conflicts; lost-update probe | **L4**: run at N=32 with repeat count; flakiness is a failure, not a retry |
| **P0.5.7** | **`CommitSequence`** primitive (A1.5 rule 3): store-native where available, coordination fallback; written as `lattice:commitSeq` on every assertion graph; **TCK: ordering suite** (monotonicity under concurrent commits) | **L4**: no two commits share a sequence; replay ordered by sequence never uses wall clock |
...
| Slice | Scope | Tests |
|---|---|---|
| **P1.2.1** | `graph_lineage` as a graph structure: lineage IRI → ordered revision IRIs with `fnd:supersededBy`; alias graph repointing primitive | **L4**: A51 forms validated; alias repoint is atomic |
...
> **Pending revision, tracked against `rdf-sparql-patterns-phase` Slice 2.** This part currently assumes SPARQL is constructed ad hoc at each write and query site (P2.1.7's RML/SHACL/SWRL emission, P2.3's plan interpreter, P2.4's governed SPARQL construction). The `ontology/persistence` substrate and `tools/persistence` compiler scoped in [rdf-sparql-patterns-phase-plan.md](rdf-sparql-patterns-phase-plan.md) and [persistence-profile-substrate.md](../sketches/persistence-profile-substrate.md) supply a declarative configuration layer and a generated-SPARQL toolchain that several slices here should compile against instead. This part must be revised once that plan's Slice 2 completes, not before, since revising it now would be guessing at an interface that slice has not yet built. Do not begin P2.1, P2.3, or P2.4 detailed design against the assumptions in this part without checking `rdf-sparql-patterns-phase`'s status first.

### P2.1 — Mapping plan compiler, C-05 (T-ING, Python + shared IR)

| Slice | Scope | Tests |
|---|---|---|
| **P2.1.1** | `plan-ir` contract: `ingestion-plan.schema.json` (route, transport, requestSchema, idempotency, steps bind/mint/assert/reference/validate, admission, provenance, writeTarget) + generated Java/Python types | **L3** |
...
| **P2.1.4** | `compile(MappingGraph, TargetClosure, GenerationProfile) → CompiledMappingSet`; digest = f(mappingSemanticHash, targetClosureHash, profileId) | **L2**: determinism, idempotence, termination (bounded on a pathological fixture) — the four `§8.4` claims become CI-asserted platform properties |
| **P2.1.5** | IRI minting strategies (`natural-key`, `derived-hash`, `surrogate`) per Appendix A, with `surrogate` requiring explicit declaration + recorded justification | **L2**: re-ingesting the same payload converges (no duplicates); `surrogate` in a convergence-required position is a compile error |
...
| **P2.1.7** | RML/SHACL/SWRL artefact emission with provenance compositionality: every emitted element back-references mapping node → intent node | **L2**: sampled emitted elements all resolve to a source; a missing back-reference fails the build |
...
| Slice | Scope | Tests |
|---|---|---|
| **P2.3.1** | Plan interpreter core (bind/mint/assert/reference), pooled frames, no per-triple store calls | **L2/L7**: identical output to the Python compiler's reference execution on the fixture corpus; allocation budget respected |
...
| **P2.3.6** | Commit: staging → A-Box batch graph + provenance + commit marker inside the store transaction (T6); reaper for `INTERRUPTED` batches | **L4**: kill mid-commit → reaper resolves by commit-marker presence; client retry is safe |
...
| Slice | Scope | Tests |
|---|---|---|
| **P2.4.1** | Typed read API generated per projection from the Projection contract at pack build | **L3/L7**: one indexed pattern per read (asserted via query plan/complexity check); meets the typed-read SLO |
| **P2.4.2** | Governed SPARQL: **gateway-constructed** `FROM`/`FROM NAMED` from `Principal` (never client-supplied); a query naming an invisible graph is rejected, not filtered | **L8**: extends P0.5.8 to the HTTP boundary |
...
| 11 | **Request Query Mapping library — unresolved.** A runtime library binding a live request, a compiled `dal:` template ([persistence-profile-substrate.md](../sketches/persistence-profile-substrate.md) §5.4), and a store SPI implementation, assuming no transport of its own. Deliberately not designed in `rdf-sparql-patterns-phase` because it needs the SPI (proposed A75) first, and a contract sketched against a nonexistent SPI would need redoing. | Blocks nothing in Phase 0 or the current `rdf-sparql-patterns-phase` plan. Blocks any Phase 2 slice that assumed this library would exist to wire ingestion or query requests to compiled templates. Design it once A75 is ratified, not before. |
| 12 | **Query Execution component — unresolved.** A component rewriting a compiled template's SPARQL dialect for a specific backend at request time ([persistence-profile-substrate.md](../sketches/persistence-profile-substrate.md) §5.5), needed because [rdf-sparql-patterns-guide.md](../../architecture/rdf-sparql-patterns-guide.md) Chapter 26 documents real per-backend dialect variance. Deferred for the same reason as row 11, plus the injection-safety question it would reopen if built without the same template discipline `tools/persistence` uses. | Same as row 11. Design it once A75 is ratified, and design it against the same template and encoder discipline as `tools/persistence`, not as a fresh string-construction path. |
