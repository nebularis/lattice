...
> **REVISED: `rdf-sparql-patterns-phase` Slices 1-2 are complete. ✅** The `ontology/persistence` substrate and `tools/persistence` compiler ([rdf-sparql-patterns-phase-plan.md](rdf-sparql-patterns-phase-plan.md), [persistence-profile-substrate.md](../sketches/persistence-profile-substrate.md), [rdf-sparql-patterns-guide.md](../architecture/rdf-sparql-patterns-guide.md)) are now available and production-tested (239 passing tests). P2.1 (RML/SHACL/SWRL emission), P2.3 (plan interpreter), and P2.4 (governed SPARQL construction) must be redesigned to generate `dal:CompiledProfile` and bind against `tools/persistence`'s template library instead of constructing SPARQL ad hoc. Before Phase 2 decomposition, Parts P2.1–P2.4 must be revisited to integrate this compiler layer. See [docs/developer/INDEX.md](../INDEX.md) for current status.
...
| 11 | **Request Query Mapping library — deferred until A75 ratification.** A runtime library binding a live request, a compiled `dal:` template (from `ontology/persistence`), and a store SPI implementation. Not designed in `rdf-sparql-patterns-phase` because it needs the SPI (proposed A75) first. See [docs/developer/INDEX.md](../INDEX.md) Part V for current status. | Blocks nothing in Phase 0 or Slice 2 of the patterns phase. Blocks any Phase 2 slice that assumed this library would exist to wire ingestion or query requests to compiled templates. Design it once A75 is ratified, not before. |
| 12 | **Query Execution component — deferred until A75 ratification.** A component rewriting a compiled `dal:` template's SPARQL dialect for a specific backend at request time, needed because [rdf-sparql-patterns-guide.md](../../architecture/rdf-sparql-patterns-guide.md) Chapter 26 documents real per-backend dialect variance. Deferred for the same reason as row 11, plus the injection-safety question it would reopen if built without the same template discipline `tools/persistence` uses. See [docs/developer/INDEX.md](../INDEX.md) Part V for current status. | Same as row 11. Design it once A75 is ratified, and design it against the same template and encoder discipline as `tools/persistence`, not as a fresh string-construction path. |
