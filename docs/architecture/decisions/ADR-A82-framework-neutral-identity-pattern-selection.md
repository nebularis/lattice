...
5. An extension to `ontology/persistence` expresses identity profiles and compiles them to minting, validation, conformance fixtures, and runtime requirements. No generic runtime component may infer identifier semantics from a prefix, class name, graph name, or current store location.
