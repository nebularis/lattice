...
| [A-84](ADR-A84-standalone-minting-libraries.md) | Standalone minting libraries | Proposed |
| [A-85](ADR-A85-vocabulary-scoped-temporal-binding-resolution.md) | Vocabulary scoped and temporal binding resolution | Accepted |
| [A-86](ADR-A86-ontology-semantic-versioning.md) | Semantic versioning for ontology documents | Accepted |
| [A-87](ADR-A87-eligibility-concept-inclusion-and-exclusion.md) | Eligibility concept inclusion and exclusion | Accepted |
| [A-88](ADR-A88-ontology-import-resolution-for-consumers.md) | Ontology import resolution for consumers | Accepted |
| [A-89](ADR-A89-eligibility-ir-concept-conditions-and-profile-aggregation.md) | Eligibility IR for concept conditions and profile aggregation | Accepted |
| [A-90](ADR-A90-eligibility-design-time-owl-class-backend.md) | Design-time OWL class backend for Eligibility | Accepted |
| [A-91](ADR-A91-eligibility-candidate-evidence-binding.md) | Eligibility candidate evidence binding | Accepted |
| [A-83](ADR-A83-test-only-reasoning-engine-isolation.md) | Test-only reasoning engine isolation | Accepted |
| [A-92](ADR-A92-derived-artefact-contract-and-prov-o-alignment.md) | Foundation derived-artefact contract and PROV-O alignment | Accepted |
| [A-93](ADR-A93-quantification-derived-rate-spaces.md) | Quantification derived rate spaces | Accepted |
| [A-94](ADR-A94-quantification-calendar-binding.md) | Quantification calendar binding | Accepted |
| [A-95](ADR-A95-quantification-alternative-bounds.md) | Quantification alternative bounds | Accepted |
| [A-96](ADR-A96-instrument-provision-attachment.md) | Instrument provision attachment | Accepted |
| [A-97](ADR-A97-mork-order-relations-in-owl-2-dl.md) | MORK order relations in OWL 2 DL | Accepted |
| [A-98](ADR-A98-applied-layout-and-insurance-modules.md) | Applied layout and insurance modules (with addendum) | Accepted |
| [A-99](ADR-A99-reference-vocabularies-and-peril-structure.md) | Reference vocabularies and peril structure | Accepted |
| [A-100](ADR-A100-hierarchical-match-over-flat-schemes.md) | Hierarchical match over schemes without a hierarchy | Proposed |
| [A-102](ADR-A102-liability-direction-from-party-roles.md) | Liability direction from party roles | Accepted |
| [A-103](ADR-A103-eligibility-set-readings.md) | Eligibility set readings | Proposed |

ADR numbering deliberately skips A-02. Eligibility (A-03–A-07 and A-07b) and Behaviour (A-08–A-11) are now assigned to the decisions authored in Gates 2 and 3. A-02 remains reserved for the document-hierarchy question folded into [../../GOVERNANCE.md](../../GOVERNANCE.md) instead of a standalone ADR. A-17–A-28 are the Surface-MORK unified projection wave, numbered continuously from A-16 per the [ADR bundle outline](../../../ontology/surface/docs/adr-bundle-outline-surface-mork-unified-projection.md); accepted following maintainer sign-off on the [delivery plan](../../../ontology/surface/docs/surface-mork-unified-projection-delivery-plan.md) Phase 0 architecture lock. A-45 through A-76 were reserved by forward reference from `solution-design-specification.md` and `Architecture Review.md`; A-48, A-50, A-51, A-54, A-57, A-59, A-62, A-63, A-65 through A-69, A-71, and A-74/A-75 are now filed as `Proposed`, drafted autonomously against that reserved block during Phase 0's P0.1 decision slices and pending human ratification (see [phase-0-status.md](../../developer/status/phase-0-status.md)). A-76 (Maven vs Gradle) remains reserved, deferred to build-skeleton slice P0.2.1. A-78 through A-80 are assigned outside that reserved block, to avoid pre-empting a number a referring document already names for a different topic. The epic's own forward reference to "ADR-A44 amended" collides with the existing, unrelated, accepted A-44 (MORK Teaching Pack); that content is filed instead as new ADR-A81, the next free number after A-80. A-82 is framework-neutral identity-pattern selection. A-83 is the test-only reasoning-engine isolation ADR proposed by the `eligibility-compiler` plan (renumbered from A-81, which that plan had claimed in collision), drafted on 2026-09-25 for the `applied-ontology-readiness` unit's OWL backend. A-84 is the standalone identity-minting libraries, proposed by the `identity-minting` unit. A-85 is Vocabulary scoped and temporal binding resolution, accepted 2026-09-25 with the `vocabulary-temporal-binding` unit's closure; follow-on consumer hardening continues under `temporal-binding-consumer-hardening`. A-86 is semantic versioning for ontology documents, from the `ontology-semantic-versioning` unit. A-87 through A-92 were proposed by the `applied-ontology-readiness` unit and accepted on 2026-09-25. A-87 records the concept inclusion and exclusion change made to Eligibility in `9a12da4`. A-93 to A-96 are that unit's Phase C decisions, accepted on 2026-09-25. A-97 makes MORK's order relations OWL 2 DL, accepted on 2026-09-25. A-98 to A-100 and A-102 are the `applied-insurance-reference` epic's Phase 0 decisions. A-98, A-99 and A-102 were accepted on 2026-09-26. A-100 was rewritten the same day, first to move it out of the applied layer, then to reduce it to Eligibility and its compilers, and A-98 gained an addendum removing the applied module it had named. A-103, set readings for evidence bindings, was proposed alongside it for the epic's Phase 3. A-101 is reserved for that epic's term parameters decision, drafted when its deferred contract phase starts. This index is updated as each is added.
