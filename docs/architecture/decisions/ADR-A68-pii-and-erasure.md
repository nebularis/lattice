...
**Status:** Proposed
**Date:** 2026-09-23
**Related:** Architecture Review §5.7 (S-9), §4.10, ADR-A51, ADR-A54, ADR-A65
...
5. Canonical IRIs never contain personal data (ADR-A51 rule 1) — use `derived-hash` minting or a per-subject reference node instead.

## Consequences

- ADR-A54's named-graph grammar must include a per-subject graph form; this is added to `docs/architecture/iri-policy.md` and the graph-name validator (P0.3.7) alongside PII-specific shapes (P0.3.8).
