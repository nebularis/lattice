...
| Vocabulary | complete | 147 lines, complete | populated (`structural.ttl`, `constraints.ttl`; `rules.ttl` deliberately documents no rule per ADR-A85) | n/a | n/a (contracts belong in consuming layers) | **T-box specified. Binding conformance package authored (ADR-A85): 12 fixtures under `examples/`, and a reference resolver + pytest suite under `tools/vocabulary/` (`mise run check:vocabulary`). Not yet executed — see `docs/developer/status/vocabulary-temporal-binding.md`.** |
...
- Consumers of `boundScheme` remain correct for unscoped inputs, but Surface and Eligibility must resolve an applicable binding first when they support scoped or temporal inputs. Demonstrated narrowly, without modifying either layer, in `ontology/vocabulary/examples/consumer-boundary.ttl` and `tools/vocabulary/tests/test_consumer_boundary.py`.
- The conformance package above is authored but **not yet executed**: this sandbox cannot install `rdflib`/`pyshacl`/`pytest` (network-restricted, see repo memory), so `mise run check:vocabulary` is a human-run handoff, not a reported pass.
