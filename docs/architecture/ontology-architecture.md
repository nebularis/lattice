...
8. **Applied-ontology readiness** (`applied-ontology-readiness`, ADRs A-87 to A-92, all Proposed). Phase A (clean examples, versioning guarantees, the import catalog) and Phase B up to evidence bindings are implemented. The design-time OWL backend (ADR-A90) waits on the ADR-A83 reasoning harness, and the Foundation derived-artefact contract (ADR-A92) and the Quantification and Instrument extensions wait on decisions. See the [status record](../developer/status/applied-ontology-readiness.md).
